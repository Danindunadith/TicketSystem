...
export default function Dashboard() {
  ...
  {stats.map((stat, index) => (
    <div 
      key={index} 
      className="bg-white rounded-xl shadow-sm p-6 transition-all duration-300 ease-in-out hover:shadow-lg hover:-translate-y-1 hover:bg-gray-50/50 group"
    >
      <div className="flex justify-between items-start">
        ...
        <div className={`${stat.color} text-white p-3 rounded-lg transition-all duration-300 ease-in-out group-hover:scale-110 group-hover:shadow-md`}>
          {stat.icon}
        </div>
      </div>
    </div>
          ...
          recentBookings.map((booking, index) => (
            ...
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full transition-all duration-300 transform group-hover:scale-105 ${
                  booking.priority === 'High' ? 'bg-red-100 text-red-800 group-hover:bg-red-200 group-hover:shadow-sm' :
